...
**Done when**:
- [ ] Registration form submits → redirects to `/auth/verify-email` with correct Cognito `signUp()` call
- [ ] Login form stores access + refresh tokens in Zustand store → redirects to `/dashboard`
- [ ] Google OAuth button initiates Cognito hosted UI redirect (correct client ID + redirect URI)
- [ ] Password reset form sends Cognito reset code and confirms new password
- [ ] Token refresh handled automatically on 401; persistent failure redirects to `/auth/login`
- [ ] All form states (loading, error, success) render correctly with design system tokens

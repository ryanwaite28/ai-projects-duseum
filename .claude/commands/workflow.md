...
- **After any deployment incident that reveals a new CDK/infra anti-pattern**: add the rule to BOTH `CLAUDE.md` § "Common Mistakes — CDK / Infrastructure" AND `devops.md` § "Blocking Criteria" + "Common Anti-Patterns". These two documents must stay in sync — one is the session-start guard, the other is the pre-change review gate. A pattern that exists in only one place will be missed half the time.
